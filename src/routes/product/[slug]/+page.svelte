<script>
  ...
  function handlePopover({ currentTarget }) {
    ...
    if ($openedPopover && popoverID !== $openedPopover.id)
      $openedPopover.classList.remove("popover--open");
...
<GlobalHeader />
<div class="page page--plp" class:isMobile>
  {#if title != "notfound"}
    <Breadcrumb>
      <Crumb>Home</Crumb>
  ...
  >
    ...
    <section class="sidebar__content">
            ...
            <Chips>
              {#each _filters as filter}
                <Chip
                  ...
                  rounded
                  onDelete={() => {
                    _filters = _filters.filter((value) => value !== filter);
                    document.querySelector(`[data-facet="${filter}"]`).click();
                  ...
                  {#each rating as j}
                    ...
                    >
                      {#if +rate >= j}
                        ...
                        />
                      {:else if !Number.isInteger(+rate) && +rate + 1 > j}
                        <defs>
                          <linearGradient id={`rad-${i}-${j}`}>
                            <stop
                              offset={`${rate.split(".")[1]}0%`}
                              stop-color="#000"
                            />
                            <stop
                              offset={`${10 - +rate.split(".")[1]}0%`}
                              stop-color="#fff"
                            />
                          </linearGradient>
                        </defs>
                        ...
                        />
                        <path
                          d="m12 15.39-3.76 2.27.99-4.28-3.32-2.88 4.38-.37L12 6.09l1.71 4.04 4.38.37-3.32 2.88.99 4.28M22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.45 4.73L5.82 21 12 17.27 18.18 21l-1.64-7.03L22 9.24Z"
                        />
                      {:else}
                        <path
                          d="m12 15.39-3.76 2.27.99-4.28-3.32-2.88 4.38-.37L12 6.09l1.71 4.04 4.38.37-3.32 2.88.99 4.28M22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.45 4.73L5.82 21 12 17.27 18.18 21l-1.64-7.03L22 9.24Z"
      ...
      <Collapsible>
        ...
        <svelte:fragment slot="content">
          <Checkbox>See what's available at Egg Hbr Twp</Checkbox>
        </svelte:fragment>
      </Collapsible>
    </section>
    {#if _filters.length}
      <footer class="sidebar__footer">
        <Button
          ...
          variant="primary"
          rounded
          on:click={handleFilterClick}>Apply Filters</Button
        >
      </footer>
    {/if}
  </div>
  {#if title != "notfound"}
    <header class="browse-header">
      ...
      <p class="browse-header__product-result-count">
        {Intl.NumberFormat().format(results)} Results
      </p>
      {#if isMobile}
        ...
        <div
          bind:this={toolbar}
          class="toolbar"
          class:toolbar--show={showToolbar}
          role="toolbar"
          aria-label=""
        >
          <div>
  ...
  <main class="content">
    {#if title != "notfound"}
      ...
      <div class="browse-footer">
        <Pagination
          {count}
          page={pageIdx + 1}
          handleChange={(idx) => (pageIdx = idx - 1)}
        />
    ...
    <Drawer open={$dataStore.open} position="right">
      ...
      <div class="drawer__content">
        <picture>
          <img
            alt={$dataStore.panel.name}
            src={$dataStore.panel.image}
            decoding="async"
            class="image"
          />
        </picture>
        <div class="product-info">
          ...
          <div class="product-card__price">
            {#if $dataStore.panel.price.sale}
              ...
              <div
                style="display:grid;align-items:center;grid-template-columns:max-content max-content;gap:3px"
              >
                <span class="product-card__price-sale strike-through"
                  >${$dataStore.panel.price.base}</span
                >
                <span style="color:#005900;font-size:12px;"
                  >{100 -
                    Math.round(
                      ($dataStore.panel.price.sale /
...
<Toast bind:this={toast}>
  ...
  <svelte:fragment slot="actions">
    <Button on:click={toast.close} style="color:#ecfe40">Retry</Button>
  </svelte:fragment>
</Toast>
